...
/*        muldvd(w0->w[i],ndash,0,&m);    Note that after this time   */
	...
	redc_count++;
	printf("%lu \n",x->len);
	if(dim1 > mr_redc_op1_len) mr_redc_op1_len = dim1;
	if(dim2 > mr_redc_op2_len) mr_redc_op2_len = dim2;
	if(buff > redc_time) redc_time = buff;
}
...
{ /* Modular multiplication using n-residues w=x*y mod n */
    ...
    miracl *mr_mip=get_mip();
#endif
    if ((x==0 || x->len==0) && x==w) return;
    if ((y==0 || y->len==0) && y==w) return;
    if (y==0 || x==0 || x->len==0 || y->len==0)
    {
        zero(w);
        return;
    }
